...
def extarct_from_response_quo(user_input):
    global chat
    try:
        if user_input.lower() == "refresh":
            # Reinitialize the chat session to clear the history and start a new chat
            chat = model.start_chat(history=[])
            print("quo chat history has been refreshed.")
        else:
            current_date = datetime.now().date()
            # Generate content using the generative model
            response = chat.send_message("text: " + user_input + ", extract keywords from the text and add them all into one list [] and give like this example '['quotation_number':'0181052024', 'contact_name':'EZEE', 'status':'open or quote_created or open/finalized or canceled or booking created or open/draft', 'shipment_type':'EXP_SEA_FCL', 'shipment_term':'FOB', 'origin':'chennai(inmaa)', 'destination':'china(jpchi)', 'company_name':'Arun Exports', 'time_period':'last year', 'from_date':'date in this format '2024-06-01'', 'to_date':'date in this format '2024-06-13''', 'month_count':'days count acording to time_period note: don't give like this '30 or 31 days' i want only one answer that also only digits and count the days not months'] there can be one keyword or many keywords also extract and give only the list no need of any aditional explanations in the response only list i want with proper key and value don't use the key as you like use what i given in example, note: remove quotations from text and if any spelling mistakes in the value of time_period or prospect correct and give, Important Note: month_count and from_date, to_date is important if there is time_period and if any name use only company_name as key not name, Very Important Note: always give the result in the list format as i shown as example and no need of key with empty value if there is value only show the key and value, important note 2: show only the key has value if no value don't give the key and value as NA and for your help today date is" + str(current_date), stream=True)
            response.resolve()
            ...
            keys_to_extract = ["quotation_number", "contact_name", "status", "shipment_type", "shipment_term", "origin", "destination"]
